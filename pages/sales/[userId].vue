<template>
  <div class="p-6 max-w-7xl mx-auto max-sm:p-0">
    ...
    <div v-if="loading" class="mt-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <!-- Stage and Priority skeleton -->
          <div class="h-6 bg-gray-300 rounded w-1/2 mb-4"></div>
          <!-- Value skeleton -->
          <div class="h-4 bg-gray-300 rounded w-3/4 mb-6"></div>
          <!-- Date skeleton -->
          <div class="h-10 bg-blue-600 rounded-md mb-2 w-full"></div>
          <!-- Update Button skeleton -->
          <div class="h-10 bg-red-600 rounded-md w-full"></div>
    ...
    <div
      v-if="showModal"
      class="fixed inset-0 flex justify-center items-center z-50 backdrop-blur-sm bg-black bg-opacity-50"
    >
      <div
        class="bg-white p-8 rounded-lg shadow-lg w-full sm:w-96 z-[100] max-sm:w-[20rem] max-sm:p-6"
      >
    ...
    >
      <div
        class="bg-white p-6 rounded-lg shadow-lg w-full sm:w-96 max-sm:w-[300px]"
      >
...
import {
  ...
  updateOpportunity,
  deleteOpportunity,
} from "~/services/salesService"; // Import service functions
...
const createOpportunityData = async () => {
  // Validate fields
  if (!newOpportunity.value.title) {
    errors.value.title = "Title is required.";
  }
  if (!newOpportunity.value.description) {
    errors.value.description = "Description is required.";
  }
  if (!newOpportunity.value.stage) {
    errors.value.stage = "Stage is required.";
  }
  if (!newOpportunity.value.value) {
    errors.value.value = "Value is required.";
  }
  if (!newOpportunity.value.priority) {
    errors.value.priority = "Priority is required.";
  }
  if (!newOpportunity.value.expectedCloseDate) {
    errors.value.expectedCloseDate = "Expected Close Date is required.";
  }

  if (
    !newOpportunity.value.title ||
    !newOpportunity.value.description ||
    !newOpportunity.value.stage ||
    !newOpportunity.value.value ||
    !newOpportunity.value.priority ||
    !newOpportunity.value.expectedCloseDate
  ) {
    return;
  }
